...
class Constant : public User {
   ...
   public:
    ...
    Constant(int32_t i32, const_str_ref name)
        : User(Type::i32_type(), vCONSTANT, name), _i32(i32), _is_zero(i32 == 0) {}

    Constant(int64_t i64, const_str_ref name)
        : User(Type::i32_type(), vCONSTANT, name), _i32(i64), _is_zero(i64 == 0) {}

    Constant(float f32, const_str_ref name)
        : User(Type::float_type(), vCONSTANT, name), _f32(f32), _is_zero(f32 == 0.0) {}

    Constant(double f64, const_str_ref name)
        : User(Type::double_type(), vCONSTANT, name), _f64(f64), _is_zero(f64 == 0.0) {}
   ...
   public:
    ...
    public:  // generate function
        ...
        static Constant* gen_f32(T val) {
            ...
            auto f32 = (float)val; auto f64 = (double)val;
            auto name = getMC(f64);
