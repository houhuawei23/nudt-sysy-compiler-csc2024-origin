...
public:
    ...
    // lowering stage
    virtual void emit_call(ir::CallInst* inst) const = 0;
    virtual void emit_prologue(MIRFunction* func) const = 0;
    virtual void emit_return(ir::ReturnInst* inst) const = 0;
    // ra stage
    virtual bool is_caller_saved(MIROperand& op) const = 0;
    virtual bool is_callee_saved(MIROperand& op) const = 0;
    // sa stage
    virtual int stack_pointer_align() const = 0;
    virtual void emit_postsa_prologue(MIRBlock* entry, int32_t stack_size) const = 0;
    virtual void emit_postsa_epilogue(MIRBlock* exit, int32_t stack_size) const = 0;
