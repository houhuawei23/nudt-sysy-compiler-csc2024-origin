...
static void fastAllocator(MIRFunction& mfunc, CodeGenContext& ctx) {
    auto liveInterval = calcLiveIntervals(mfunc, ctx);  // 计算变量的活跃区间
    ...
    std::unordered_map<MIROperand*, MIROperand*, MIROperandHasher> stack_map;        // 栈映射 (全局分析) - 将跨多个块的虚拟寄存器spill到内存
    ...
    /* 局部寄存器分配 -- 考虑在每个块内进行单独的分析 */
    for (auto& block : mfunc.blocks()) {
        std::unordered_map<MIROperand*, MIROperand*, MIROperandHasher> local_stack_map;  // 栈映射 (局部分析) - 物理寄存器有限, 将额外的虚拟寄存器spill到内存
        std::unordered_map<MIROperand*, std::vector<MIROperand*>, MIROperandHasher> cur_map;  // 当前块中, 每个虚拟寄存器的映射 -> 栈 or 物理寄存器
        std::unordered_map<MIROperand*, MIROperand*, MIROperandHasher> phy_map;  // 当前块中, 每个虚拟寄存器映射所对应的物理寄存器的映射
    ...
    //     std::unordered_set<MIROperand*, MIROperandHasher> dirty_regs;
        ...
        auto collect_underRenamedISARegs = [&](MIRInstList::iterator it) {
            while (it != instructions.end()) {
                ...
                for (uint32_t idx = 0; idx < instInfo.operand_num(); idx++) {
                    auto op = inst->operand(idx);
                    if (isOperandISAReg(op) && !ctx.registerInfo->is_zero_reg(op->reg()) && is_allocatableType(op->type()) && (instInfo.operand_flag(idx) & OperandFlagUse)) {
