 ...
 * @note: 
 *      Register Selector (分为General-Purpose Register and Floating Point Register)
 */
constexpr uint32_t maxRegisterCount = 60;
class RegisterSelector {
    int64_t _mFree;  // 使用位集合的形式跟踪所有可用的寄存器: 如果位为1, 表示对应的寄存器是空闲的
    std::array<uint32_t, maxRegisterCount> _mIdx2Reg;  // 将数组索引映射到寄存器编号
 ...
 * @note: 
 *      Multi-Class Register Selector (include General-Purpose Registers and Floating-Point Registers)
 */
class MultiClassRegisterSelector final {
    ...
    public: 
        MultiClassRegisterSelector(class TargetRegisterInfo& info) : _mRegisterInfo(info) {
            for (uint32_t idx = 0; idx < info.get_alloca_class_cnt(); idx++) {
                _mSelectors.push_back(std::make_unique<RegisterSelector>(info.get_allocation_list(idx)));
            }
        }
