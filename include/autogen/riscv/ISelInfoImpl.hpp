...
static bool matchInstJump(
    MIRInst* inst, MIROperand& target)
{
...
static bool matchInstBranch(
    MIRInst* inst, MIROperand& cond, MIROperand& target, MIROperand& prob)
{
    if(inst->opcode() != InstBranch)
        return false;
    cond = inst->operand(0);target = inst->operand(1);prob = inst->operand(2);
    return true;
}



static bool matchInstUnreachable(
    MIRInst* inst)
{
...
static bool matchInstLoad(
    MIRInst* inst, MIROperand& dst, MIROperand& addr, MIROperand& align)
{
    if(inst->opcode() != InstLoad)
        return false;
    dst = inst->operand(0);addr = inst->operand(1);align = inst->operand(2);
    return true;
}



static bool matchInstStore(
    MIRInst* inst, MIROperand& addr, MIROperand& src, MIROperand& align)
{
    if(inst->opcode() != InstStore)
        return false;
    addr = inst->operand(0);src = inst->operand(1);align = inst->operand(2);
    return true;
}
...
static bool matchInstAdd(
    MIRInst* inst, MIROperand& dst, MIROperand& src1, MIROperand& src2)
{
    if(inst->opcode() != InstAdd)
        return false;
    dst = inst->operand(0);src1 = inst->operand(1);src2 = inst->operand(2);
    return true;
}



static bool matchInstSub(
    MIRInst* inst, MIROperand& dst, MIROperand& src1, MIROperand& src2)
{
...
static bool matchInstNeg(
    MIRInst* inst, MIROperand& dst, MIROperand& src)
{
    if(inst->opcode() != InstNeg)
        return false;
    dst = inst->operand(0);src = inst->operand(1);
    return true;
}



static bool matchInstAbs(
    MIRInst* inst, MIROperand& dst, MIROperand& src)
{
...
static bool matchInstFFma(
    MIRInst* inst, MIROperand& dst, MIROperand& src1, MIROperand& src2, MIROperand& acc)
{
    if(inst->opcode() != InstFFma)
        return false;
    dst = inst->operand(0);src1 = inst->operand(1);src2 = inst->operand(2);acc = inst->operand(3);
    return true;
}



static bool matchInstICmp(
    MIRInst* inst, MIROperand& dst, MIROperand& src1, MIROperand& src2, MIROperand& op)
{
    if(inst->opcode() != InstICmp)
        return false;
    dst = inst->operand(0);src1 = inst->operand(1);src2 = inst->operand(2);op = inst->operand(3);
    return true;
}



static bool matchInstFCmp(
    MIRInst* inst, MIROperand& dst, MIROperand& src1, MIROperand& src2, MIROperand& op)
{
...
static bool matchInstFCast(
    MIRInst* inst, MIROperand& dst)
{
...
static bool matchInstSelect(
    MIRInst* inst, MIROperand& dst, MIROperand& cond, MIROperand& src1, MIROperand& src2)
{
    if(inst->opcode() != InstSelect)
        return false;
    dst = inst->operand(0);cond = inst->operand(1);src1 = inst->operand(2);src2 = inst->operand(3);
    return true;
}



static bool matchInstLoadGlobalAddress(
    MIRInst* inst, MIROperand& dst, MIROperand& addr)
{
    if(inst->opcode() != InstLoadGlobalAddress)
        return false;
    dst = inst->operand(0);addr = inst->operand(1);
    return true;
}



static bool matchInstLoadImm(
    MIRInst* inst, MIROperand& dst, MIROperand& imm)
{
    if(inst->opcode() != InstLoadImm)
        return false;
    dst = inst->operand(0);imm = inst->operand(1);
    return true;
}



static bool matchInstLoadStackObjectAddr(
    MIRInst* inst, MIROperand& dst, MIROperand& obj)
{
    if(inst->opcode() != InstLoadStackObjectAddr)
        return false;
    dst = inst->operand(0);obj = inst->operand(1);
    return true;
}



static bool matchInstCopyFromReg(
    MIRInst* inst, MIROperand& dst, MIROperand& src)
{
...
static bool matchInstCopyToReg(
    MIRInst* inst, MIROperand& dst, MIROperand& src)
{
...
static bool matchInstLoadImmToReg(
    MIRInst* inst, MIROperand& dst, MIROperand& imm)
{
...
static bool matchInstLoadRegFromStack(
    MIRInst* inst, MIROperand& dst, MIROperand& obj)
{
...
static bool matchInstStoreRegToStack(
    MIRInst* inst, MIROperand& obj, MIROperand& src)
{
    if(inst->opcode() != InstStoreRegToStack)
        return false;
    obj = inst->operand(0);src = inst->operand(1);
    return true;
}



static bool matchInstReturn(
    MIRInst* inst)
{
...
static bool matchAndSelectPattern1 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    /** Match Inst **/
        /* match inst InstLoadGlobalAddress */
            MIROperand op1;MIROperand op2;
            if (not matchInstLoadGlobalAddress(
                inst1, op1, op2))  { return false; }
    ...
    auto op5 = (op2);
    /* select inst LLA */
            auto inst2 = ctx.insertMIRInst(LLA, {
                op4, op5
            });
            
    
        /* Replace Operand */
        ctx.replace_operand(ctx.getInstDefOperand(inst1), 
                            ctx.getInstDefOperand(inst2));
    ctx.remove_inst(inst1);
    return true;
} 
static bool matchAndSelectPattern2 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstLoadGlobalAddress;
    ...
    /* select inst AUIPC */
            auto inst2 = ctx.insertMIRInst(AUIPC, {
                op6, op7
            });
    ...
    /* select inst ADDI */
            auto inst3 = ctx.insertMIRInst(ADDI, {
                op4, op5, op8
            });
...
static bool matchAndSelectPattern3 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    /** Match Inst **/
        /* match inst InstLoad */
            MIROperand op1;MIROperand op2;MIROperand op3;
            if (not matchInstLoad(
                inst1, op1, op2, op3))  { return false; }
    
        /* match predicate for operands  */
            MIROperand op4;MIROperand op5;if(not (selectAddrOffset(op2, ctx, op4, op5))) {
                return false;
            }
    ...
    /* select inst getLoadOpcode(op1) */
            auto inst2 = ctx.insertMIRInst(getLoadOpcode(op1), {
                op7, op9, op8
            });
...
static bool matchAndSelectPattern4 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    /** Match Inst **/
        /* match inst InstStore */
            MIROperand op1;MIROperand op2;MIROperand op3;
            if (not matchInstStore(
                inst1, op1, op2, op3))  { return false; }
    
        /* match predicate for operands  */
            MIROperand op4;MIROperand op5;if(not (isOperandVRegORISAReg(op2) && selectAddrOffset(op1, ctx, op4, op5))) {
                return false;
            }
    ...
    /* select inst getStoreOpcode(op2) */
            auto inst2 = ctx.insertMIRInst(getStoreOpcode(op2), {
                op7, op9, op8
            });
...
static bool matchAndSelectPattern5 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    /** Match Inst **/
        /* match inst InstJump */
            MIROperand op1;
            if (not matchInstJump(
                inst1, op1))  { return false; }
    

    /** Select Inst **/
    auto op3 = (op1);
    /* select inst J */
            auto inst2 = ctx.insertMIRInst(J, {
                op3
            });
...
static bool matchAndSelectPattern6 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    /* select inst MV */
            auto inst2 = ctx.insertMIRInst(MV, {
                op4, op5
            });
...
static bool matchAndSelectPattern8 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstLoadImm;
    ...
    /* select inst LoadImm12 */
            auto inst2 = ctx.insertMIRInst(LoadImm12, {
                op4, op5
            });
...
static bool matchAndSelectPattern10 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstLoadImm;
    ...
    /* select inst LoadImm32 */
            auto inst2 = ctx.insertMIRInst(LoadImm32, {
                op4, op5
            });
...
static bool matchAndSelectPattern7 (
    MIRInst* inst1,
    ISelContext& ctx)
{
...
static bool matchAndSelectPattern9 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstLoadImmToReg;
...
static bool matchAndSelectPattern11 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstLoadImmToReg;
...
static bool matchAndSelectPattern12 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    /** Match Inst **/
        /* match inst InstAdd */
            MIROperand op1;MIROperand op2;MIROperand op3;
            if (not matchInstAdd(
                inst1, op1, op2, op3))  { return false; }/* lookup inst that define the operand2 */
                auto inst2 = ctx.lookupDefInst(op2);
                if (not inst2) { return false; }
    
        /* match inst InstShl */
            MIROperand op4;MIROperand op5;MIROperand op6;
            if (not matchInstShl(
                inst2, op4, op5, op6))  { return false; }
    
        /* match predicate for operands  */
            if(not (isOperandIReg(op1) && isOperandIReg(op5) && isOperandIReg(op3) && (op6).isImm() && (op6).imm() == 1)) {
                return false;
            }
    ...
    auto op10 = (op3);
    /* select inst SH1ADD */
            auto inst3 = ctx.insertMIRInst(SH1ADD, {
                op8, op9, op10
            });
...
static bool matchAndSelectPattern13 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstAdd;
    /** Match Inst **/
        /* match inst InstAdd */
            MIROperand op1;MIROperand op2;MIROperand op3;
            if (not matchInstAdd(
                inst1, op1, op2, op3))  { return false; }/* lookup inst that define the operand3 */
        ...
        /* match predicate for operands  */
            if(not (isOperandIReg(op1) && isOperandIReg(op2) && isOperandIReg(op5) && (op6).isImm() && (op6).imm() == 1)) {
                return false;
            }
...
static bool matchAndSelectPattern14 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstAdd;
    /** Match Inst **/
        ...
        /* match predicate for operands  */
            if(not (isOperandIReg(op1) && isOperandIReg(op5) && isOperandIReg(op3) && (op6).isImm() && (op6).imm() == 2)) {
                return false;
            }
    ...
    auto op10 = (op3);
    /* select inst SH2ADD */
            auto inst3 = ctx.insertMIRInst(SH2ADD, {
                op8, op9, op10
            });
...
static bool matchAndSelectPattern15 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstAdd;
    /** Match Inst **/
        ...
        /* match predicate for operands  */
            if(not (isOperandIReg(op1) && isOperandIReg(op2) && isOperandIReg(op5) && (op6).isImm() && (op6).imm() == 2)) {
                return false;
            }
...
static bool matchAndSelectPattern16 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstAdd;
    /** Match Inst **/
        ...
        /* match predicate for operands  */
            if(not (isOperandIReg(op1) && isOperandIReg(op5) && isOperandIReg(op3) && (op6).isImm() && (op6).imm() == 3)) {
                return false;
            }
    ...
    auto op10 = (op3);
    /* select inst SH3ADD */
            auto inst3 = ctx.insertMIRInst(SH3ADD, {
                op8, op9, op10
            });
...
static bool matchAndSelectPattern17 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstAdd;
    /** Match Inst **/
        ...
        /* match predicate for operands  */
            if(not (isOperandIReg(op1) && isOperandIReg(op2) && isOperandIReg(op5) && (op6).isImm() && (op6).imm() == 3)) {
                return false;
            }
...
static bool matchAndSelectPattern18 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstAdd;
    ...
    auto op7 = (op3);
    /* select inst ADDI */
            auto inst2 = ctx.insertMIRInst(ADDI, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern19 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstAdd;
    ...
    auto op7 = (op3);
    /* select inst ADD */
            auto inst2 = ctx.insertMIRInst(ADD, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern20 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstAdd;
    ...
    auto op7 = (op3);
    /* select inst ADDIW */
            auto inst2 = ctx.insertMIRInst(ADDIW, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern21 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstAdd;
    ...
    auto op7 = (op3);
    /* select inst ADDW */
            auto inst2 = ctx.insertMIRInst(ADDW, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern22 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op7 = (op3);
    /* select inst SUB */
            auto inst2 = ctx.insertMIRInst(SUB, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern23 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstSub;
    ...
    auto op7 = (op3);
    /* select inst SUBW */
            auto inst2 = ctx.insertMIRInst(SUBW, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern24 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op7 = (op3);
    /* select inst MUL */
            auto inst2 = ctx.insertMIRInst(MUL, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern25 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstMul;
    ...
    auto op7 = (op3);
    /* select inst MULW */
            auto inst2 = ctx.insertMIRInst(MULW, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern26 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op7 = (op3);
    /* select inst DIVU */
            auto inst2 = ctx.insertMIRInst(DIVU, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern27 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstUDiv;
    ...
    auto op7 = (op3);
    /* select inst DIVUW */
            auto inst2 = ctx.insertMIRInst(DIVUW, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern28 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op7 = (op3);
    /* select inst REMU */
            auto inst2 = ctx.insertMIRInst(REMU, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern29 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstURem;
    ...
    auto op7 = (op3);
    /* select inst REMUW */
            auto inst2 = ctx.insertMIRInst(REMUW, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern30 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    /* select inst getIntegerBinaryRegOpcode(rootOpcode) */
            auto inst2 = ctx.insertMIRInst(getIntegerBinaryRegOpcode(rootOpcode), {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern33 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstAnd;
    ...
    /* select inst getIntegerBinaryImmOpcode(rootOpcode) */
            auto inst2 = ctx.insertMIRInst(getIntegerBinaryImmOpcode(rootOpcode), {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern31 (
    MIRInst* inst1,
    ISelContext& ctx)
{
...
static bool matchAndSelectPattern34 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstOr;
...
static bool matchAndSelectPattern32 (
    MIRInst* inst1,
    ISelContext& ctx)
{
...
static bool matchAndSelectPattern35 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstXor;
...
static bool matchAndSelectPattern36 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op7 = (op3);
    /* select inst SLLI */
            auto inst2 = ctx.insertMIRInst(SLLI, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern37 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstShl;
    ...
    auto op7 = (op3);
    /* select inst SLL */
            auto inst2 = ctx.insertMIRInst(SLL, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern38 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstShl;
    ...
    auto op7 = (op3);
    /* select inst SLLIW */
            auto inst2 = ctx.insertMIRInst(SLLIW, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern39 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstShl;
    ...
    auto op7 = (op3);
    /* select inst SLLW */
            auto inst2 = ctx.insertMIRInst(SLLW, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern40 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op7 = (op3);
    /* select inst SRL */
            auto inst2 = ctx.insertMIRInst(SRL, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern41 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstLShr;
    ...
    auto op7 = (op3);
    /* select inst SRLW */
            auto inst2 = ctx.insertMIRInst(SRLW, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern42 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op7 = (op3);
    /* select inst SRAI */
            auto inst2 = ctx.insertMIRInst(SRAI, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern43 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstAShr;
    ...
    auto op7 = (op3);
    /* select inst SRA */
            auto inst2 = ctx.insertMIRInst(SRA, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern44 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstAShr;
    ...
    auto op7 = (op3);
    /* select inst SRAIW */
            auto inst2 = ctx.insertMIRInst(SRAIW, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern45 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstAShr;
    ...
    auto op7 = (op3);
    /* select inst SRAW */
            auto inst2 = ctx.insertMIRInst(SRAW, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern46 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op7 = (op3);
    /* select inst DIV */
            auto inst2 = ctx.insertMIRInst(DIV, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern47 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstSDiv;
    ...
    auto op7 = (op3);
    /* select inst DIVW */
            auto inst2 = ctx.insertMIRInst(DIVW, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern48 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op7 = (op3);
    /* select inst REM */
            auto inst2 = ctx.insertMIRInst(REM, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern49 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstSRem;
    ...
    auto op7 = (op3);
    /* select inst REMW */
            auto inst2 = ctx.insertMIRInst(REMW, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern50 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op7 = (op3);
    /* select inst MIN */
            auto inst2 = ctx.insertMIRInst(MIN, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern51 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op7 = (op3);
    /* select inst MAX */
            auto inst2 = ctx.insertMIRInst(MAX, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern52 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op9 = (op2);
    /* select inst MAX */
            auto inst3 = ctx.insertMIRInst(MAX, {
                op4, op5, op9
            });
...
static bool matchAndSelectPattern53 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    /** Match Inst **/
        /* match inst InstICmp */
            MIROperand op1;MIROperand op2;MIROperand op3;MIROperand op4;
            if (not matchInstICmp(
                inst1, op1, op2, op3, op4))  { return false; }
    
        /* match predicate for operands  */
            if(not (isOperandIReg(op2) && isOperandIReg(op3) && isCompareOp(op4, CompareOp::ICmpSignedLessThan))) {
                return false;
            }
    ...
    auto op8 = (op3);
    /* select inst SLT */
            auto inst2 = ctx.insertMIRInst(SLT, {
                op6, op7, op8
            });
...
static bool matchAndSelectPattern54 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstICmp;
    /** Match Inst **/
        ...
        /* match predicate for operands  */
            if(not (isOperandIReg(op2) && isOperandIReg(op3) && isCompareOp(op4, CompareOp::ICmpSignedGreaterEqual))) {
                return false;
            }
    ...
    /* select inst XORI */
            auto inst3 = ctx.insertMIRInst(XORI, {
                op6, op7, op11
            });
...
static bool matchAndSelectPattern55 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstICmp;
    /** Match Inst **/
        ...
        /* match predicate for operands  */
            if(not (isOperandIReg(op2) && isOperandIReg(op3) && isCompareOp(op4, CompareOp::ICmpUnsignedLessThan))) {
                return false;
            }
    ...
    auto op8 = (op3);
    /* select inst SLTU */
            auto inst2 = ctx.insertMIRInst(SLTU, {
                op6, op7, op8
            });
...
static bool matchAndSelectPattern56 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstICmp;
    /** Match Inst **/
        ...
        /* match predicate for operands  */
            if(not (isOperandIReg(op2) && isOperandIReg(op3) && isCompareOp(op4, CompareOp::ICmpSignedGreaterThan))) {
                return false;
            }
...
static bool matchAndSelectPattern57 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstICmp;
    /** Match Inst **/
        ...
        /* match predicate for operands  */
            if(not (isOperandIReg(op2) && isOperandIReg(op3) && isCompareOp(op4, CompareOp::ICmpUnsignedGreaterThan))) {
                return false;
            }
...
static bool matchAndSelectPattern58 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstICmp;
    ...
    auto op8 = (getOne(op3));
    /* select inst SLTIU */
            auto inst2 = ctx.insertMIRInst(SLTIU, {
                op6, op7, op8
            });
...
static bool matchAndSelectPattern59 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstICmp;
...
static bool matchAndSelectPattern60 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    uint32_t rootOpcode = InstICmp;
    /** Match Inst **/
        ...
        /* match predicate for operands  */
            if(not (isOperandIReg(op2) && isOperandIReg(op3) && isCompareOp(op4, CompareOp::ICmpSignedLessEqual))) {
                return false;
            }
...
static bool matchAndSelectPattern61 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op8 = (op3);
    /* select inst BNE */
            auto inst2 = ctx.insertMIRInst(BNE, {
                op5, op6, op7, op8
            });
...
static bool matchAndSelectPattern62 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    /* select inst FCVT_W_S */
            auto inst2 = ctx.insertMIRInst(FCVT_W_S, {
                op4, op5
            });
...
static bool matchAndSelectPattern63 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    /* select inst FCVT_S_W */
            auto inst2 = ctx.insertMIRInst(FCVT_S_W, {
                op4, op5
            });
...
static bool matchAndSelectPattern64 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    /** Match Inst **/
        /* match inst InstFCmp */
            MIROperand op1;MIROperand op2;MIROperand op3;MIROperand op4;
            if (not matchInstFCmp(
                inst1, op1, op2, op3, op4))  { return false; }
    
        /* match predicate for operands  */
            MIROperand op5;MIROperand op6;MIROperand op7;if(not (selectFCmpOpcode(op4, op2, op3, op5, op6, op7))) {
                return false;
            }
    ...
    /* select inst static_cast<uint32_t>((op7).imm()) */
            auto inst2 = ctx.insertMIRInst(static_cast<uint32_t>((op7).imm()), {
                op9, op10, op11
            });
...
static bool matchAndSelectPattern65 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op5 = (op2);
    /* select inst FNEG_S */
            auto inst2 = ctx.insertMIRInst(FNEG_S, {
                op4, op5
            });
...
static bool matchAndSelectPattern66 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op7 = (op3);
    /* select inst FADD_S */
            auto inst2 = ctx.insertMIRInst(FADD_S, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern67 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op7 = (op3);
    /* select inst FSUB_S */
            auto inst2 = ctx.insertMIRInst(FSUB_S, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern68 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op7 = (op3);
    /* select inst FMUL_S */
            auto inst2 = ctx.insertMIRInst(FMUL_S, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern69 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    auto op7 = (op3);
    /* select inst FDIV_S */
            auto inst2 = ctx.insertMIRInst(FDIV_S, {
                op5, op6, op7
            });
...
static bool matchAndSelectPattern70 (
    MIRInst* inst1,
    ISelContext& ctx)
{
    ...
    /* select inst InstCopy */
            auto inst2 = ctx.insertMIRInst(InstCopy, {
                op4, op5
            });
