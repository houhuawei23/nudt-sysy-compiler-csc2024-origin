...
namespace sysy
{
    std::any SysYIRGenerator::visitNumberExp(SysYParser::NumberExpContext *ctx)
    {
        ...
        else if (auto fctx = ctx->number()->FLITERAL())
        { // float
    ...
    std::any SysYIRGenerator::visitUnaryExp(SysYParser::UnaryExpContext *ctx)
    {
        ...
        //         if (typeid(ans) == typeid(float)) res = ir::Constant::gen(ans, ir::getMC(ans));
        //         else res = ir::Constant::gen(ans);
        //     } else if (ctx->ADD()) {
        //         // TODO
        //     } else {
        //         // TODO
        ...
        //     if (op->is_int()) {
        //         // int32 类型
        //         res = _builder.create_fneg(ir::Type::int_type(), op, _builder.getvarname());
        //     } else {
        //         // float 类型
        //         res = _builder.create_fneg(ir::Type::float_type(), op, _builder.getvarname());
        //     }
        //     //! TODO
        // }
    ...
    std::any SysYIRGenerator::visitMultiplicativeExp(SysYParser::MultiplicativeExpContext *ctx)
    {
        ...
        { //! 2. 变量 -> 生成 MUL | FMUL | UDIV | SDIV | FDIV | UREM | SREM | FREM 指令
            if (op1->is_int() && op2->is_int())
            {
                // int32 类型
                if (ctx->MUL())
                    res = _builder.create_mul(ir::Type::int_type(), op1, op2, _builder.getvarname());
                else if (ctx->DIV())
                    res = _builder.create_div(ir::Type::int_type(), op1, op2, _builder.getvarname());
                else
                    res = _builder.create_rem(ir::Type::int_type(), op1, op2, _builder.getvarname());
            }
            else if (op1->is_float() && op2->is_float())
            {
                // float 类型
                if (ctx->MUL())
                    res = _builder.create_mul(ir::Type::float_type(), op1, op2, _builder.getvarname());
                else if (ctx->DIV())
                    res = _builder.create_div(ir::Type::float_type(), op1, op2, _builder.getvarname());
                else
                    res = _builder.create_rem(ir::Type::float_type(), op1, op2, _builder.getvarname());
            }
            else
            {
                // 需要进行隐式类型转换
                ...
                {
                    auto sitof = _builder.create_sitof(ir::Type::float_type(), op1, _builder.getvarname());
                    if (ctx->MUL())
                        res = _builder.create_mul(ir::Type::float_type(), sitof, op2, _builder.getvarname());
                    else if (ctx->DIV())
                        res = _builder.create_div(ir::Type::float_type(), sitof, op2, _builder.getvarname());
                    else
                        res = _builder.create_rem(ir::Type::float_type(), sitof, op2, _builder.getvarname());
                }
                else
                {
                    auto sitof = _builder.create_sitof(ir::Type::float_type(), op2, _builder.getvarname());
                    if (ctx->MUL())
                        res = _builder.create_mul(ir::Type::float_type(), op1, sitof, _builder.getvarname());
                    else if (ctx->DIV())
                        res = _builder.create_div(ir::Type::float_type(), op1, sitof, _builder.getvarname());
                    else
                        res = _builder.create_rem(ir::Type::float_type(), op1, sitof, _builder.getvarname());
                }
            }
        }
        return res;
    ...
    std::any SysYIRGenerator::visitAdditiveExp(SysYParser::AdditiveExpContext *ctx)
    {
        ...
        { //! 2. 变量 -> 生成 ADD | fADD | SUB | fSUB 指令
            if (op1->is_int() && op2->is_int())
            {
                // int32 类型加减
                if (ctx->SUB())
                    res = _builder.create_sub(ir::Type::int_type(), op1, op2, _builder.getvarname());
                else
                    res = _builder.create_add(ir::Type::int_type(), op1, op2, _builder.getvarname());
            }
            else if (op1->is_float() && op2->is_float())
            {
                // float 类型加减
                if (ctx->SUB())
                    res = _builder.create_sub(ir::Type::float_type(), op1, op2, _builder.getvarname());
                else
                    res = _builder.create_add(ir::Type::float_type(), op1, op2, _builder.getvarname());
            }
            else
            {
                ...
                if (op1->is_int())
                {
                    op1 = _builder.create_sitof(ir::Type::float_type(), op1, _builder.getvarname());
                }
                if (op2->is_int())
                {
                    op2 = _builder.create_sitof(ir::Type::float_type(), op2, _builder.getvarname());
                }
                if (ctx->SUB())
                    res = _builder.create_sub(ir::Type::float_type(), op1, op2, _builder.getvarname());
                else
