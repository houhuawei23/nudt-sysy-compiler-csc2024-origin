...
int SCEV::getIndVarIterCnt(ir::Loop* lp, ir::indVar* idv){
    if(not idv->isEndVarConst())return -1;//cannot calc iter cnt for non constant endvar
    int beginVar=idv->getBeginI32();
    int stepVar=idv->getStepI32();
    int endVar=idv->getEndVarI32();
    auto cmpinst=idv->cmpInst();
    auto iterInst=idv->iterInst();
    if(iterInst->valueId()==ir::vADD){
        if(cmpinst->valueId()==ir::vISGT){
            
        }
        else if(cmpinst->valueId()==ir::vISGE){

        }
        else if(cmpinst->valueId()==ir::vISLT){

        }
        else if(cmpinst->valueId()==ir::vISLE){

        }
        else if(cmpinst->valueId()==ir::vIEQ){

        }
        else if(cmpinst->valueId()==ir::vINE){

        }
        else{
            assert(false&&"invalid cmpinst operator in indvar!");
        }
    }
