...
ir::BasicBlock* simplifyCFG::getMergeBlock(ir::BasicBlock* bb) {  // condition 2
    if (bb->next_blocks().size() == 1)
        if ((*(bb->next_blocks().begin()))->pre_blocks().size() == 1) return *(bb->next_blocks().begin());
...
bool simplifyCFG::removeSingleBrBlock(ir::Function* func) {
    ...
    */
    ...
    while (not worklist.empty()) {
        ...
        std::set<ir::BasicBlock*> curBBpreBBs =
          std::set<ir::BasicBlock*>(curBB->pre_blocks().begin(), curBB->pre_blocks().end());
        std::set<ir::BasicBlock*> destBBpreBBs =
        ...
        std::set_intersection(curBBpreBBs.begin(), curBBpreBBs.end(), destBBpreBBs.begin(), destBBpreBBs.end(),
                              std::insert_iterator<std::vector<ir::BasicBlock*>>(curBBdestBBPreBBIntersection,
                                                                                 curBBdestBBPreBBIntersection.begin()));
