...
void SROA::runOnLoop(ir::Loop* lp){
    ...
    for(auto gep:SROAGeps){
        ...
        if(domctx->dominate(gep->block(),lp->header()) and gep->block()!=lp->header()){
            ...
            res=replaceAllUseInLpForLpI(gep,lp,newAlloca,isReadGep and not isWriteGep,isWriteGep and not isReadGep);
            if(res){
            ...
            bool isReadGep=depInfoForLp->getIsSubAddrRead(gep);
            bool isWriteGep=depInfoForLp->getIsSubAddrWrite(gep);
            res=replaceAllUseInLpIdv(gep,lp,newAlloca,isReadGep and not isWriteGep,isWriteGep and not isReadGep);
            if(res){
                processedSubAddrs.insert(gep);
                std::cerr<<"SROA lift"<<std::endl;
            }
