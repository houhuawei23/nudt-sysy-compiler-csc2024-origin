...
namespace pass {
Function* StatelessCache::getLookupFunction(Module* module, ArrayType* lutType) {
  ...
  const auto funcType = FunctionType::gen(PointerType::gen(i32), {lutType, i32, i32});
...
bool StatelessCache::runImpl(ir::Function* func, TopAnalysisInfoManager* tp) {
  ...
  const auto lookupFunc = getLookupFunction(func->module(), lutType);
