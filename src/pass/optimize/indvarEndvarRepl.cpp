...
void idvEdvRepl::runOnLoop(ir::Loop* lp){
...
bool idvEdvRepl::isSimplyLoopInvariant(ir::Loop* lp,ir::Value* val){
...
int idvEdvRepl::getConstantEndvarIndVarIterCnt(ir::Loop* lp,ir::IndVar* idv){
    //-1 不能计算
...
void idvEdvRepl::normalizeIcmpAndBr(ir::Loop* lp,ir::IndVar* idv){
    auto endvar=idv->endValue();
...
void idvEdvRepl::exchangeIcmpOp(ir::ICmpInst* icmpInst){
...
//翻转这个Icmp的符号使得原意不变
void idvEdvRepl::reverseIcmpOp(ir::ICmpInst* icmpInst){
    switch (icmpInst->valueId())
    {
    case ir::vIEQ:
        break;
...
//交换brcond的跳转两个目标
void idvEdvRepl::exchangeBrDest(ir::BranchInst* brInst){
...
bool idvEdvRepl::isSimplyNotInLoop(ir::Loop* lp,ir::Value* val){
