...
int32_t RISCVFrameInfo::insert_prologue_epilogue(
    MIRFunction* func,
...
void RISCVFrameInfo::emit_call(ir::CallInst* inst,
  ...
  for (uint32_t idx = 0; idx < inst->rargs().size(); idx++) {
    ...
    if (offset < passingByRegBase) { /* pass by stack */
      auto obj = mirFunc->add_stack_obj(lowering_ctx._code_gen_ctx->next_id(),
                                        size, align, offset,
        ...
        if (inst->opcode() == RISCV::AUIPC) {
          ...
          if (instIter == insts.begin() && blockIter != func.blocks().begin()) {
            ...
            auto newBlock = std::make_unique<MIRBlock>(
                &func, "pcrel" + std::to_string(ctx.next_id_label()));
          ...
          for (uint32_t idx = 0; idx < instInfo.operand_num(); idx++) {
            ...
            if (operand->is_reloc() &&
              ...
              auto op = getLowBits(MIROperand::as_reloc(pcrelBlock));
